...
| --- | --- | --- | --- | --- | --- | --- |
| Agent 对话 | **可选 ACP Agent 对话：API Key 网关登录、每个项目多会话与历史加载、流式输出、工具权限、取消和进程清理**<br><sub>agent-acp-conversation</sub> | ✅ 已实现<br><sub>🔍 待验证</sub><br><sub>`macos/Sources/LitheAgentConversationModule`、`macos/Sources/Lithe/Views/Agent`、`rust/lithe-agent-host`</sub> | 🟡 部分实现<br><sub>🔍 待验证</sub><br><sub>`rust/lithe-agent-host`</sub> | Agent | 在 macOS 安装 codex-acp 并选择 Responses 协议的 AI 服务商，验证网关登录、连续对话、权限选择、刚发送即取消后可继续对话、历史会话列表与加载、切换项目标签后后台会话继续运行，以及禁用功能、关闭项目或退出应用后 Agent 进程全部退出；Windows 待接入 UI 和宿主。 |  |
| Agent 对话 | **Agent 与 AI 服务商管理：Node.js/npm 检测、ACP 适配器一键安装与卸载、每个 Agent 选择服务商与模型、服务商独立编辑**<br><sub>agent-management</sub> | ✅ 已实现<br><sub>🔍 待验证</sub><br><sub>`macos/Sources/Lithe/Views/App/AgentsSettingsView.swift`、`macos/Sources/Lithe/Application/Features/AgentManagementFeatureModel.swift`、`rust/lithe-agent-host/src/install.rs`、`rust/lithe-core/src/agent/mod.rs`</sub> | 🟡 部分实现<br><sub>🔍 待验证</sub><br><sub>`rust/lithe-core/src/agent/mod.rs`</sub> | Agent | 在 macOS 打开 设置 › Agents：确认显示 Node.js 与 npm 版本，Node 缺失或版本过低时只提示；一键安装、取消安装、更新与卸载 Codex 适配器；为 Codex 选择 Responses 协议服务商后在 Agent 面板对话；在 AI Providers 切换编辑对象不改变提交信息所用服务商；Windows 待接入设置界面。 |  |
