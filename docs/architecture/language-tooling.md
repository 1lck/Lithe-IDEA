...
MyBatis Mapper 到 XML 的跳转不经过 JDT LS。`mybatis.index` 用 Java 接口方法名
与 XML `<mapper namespace>` 中的 statement `id` 配对；Go to Definition 和
Cmd/Ctrl-click 在调用语言服务器之前使用该索引。没有配对 XML 的方法继续走
普通 LSP 定义跳转。
