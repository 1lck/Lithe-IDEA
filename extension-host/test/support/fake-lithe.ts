...
export class FakeLithe {
    ...
    static launch(directory = fs.mkdtempSync(path.join(os.tmpdir(), 'lithe-extension-host-'))): FakeLithe {
        ...
        if (!fs.existsSync(HOST_ENTRY)) {
            throw new Error(`Build the host first (bun run build); ${HOST_ENTRY} is missing.`);
        }
        ...
        const child = spawn(node, [HOST_ENTRY], {
