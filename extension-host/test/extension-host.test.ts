...
describe('commands', () => {
    ...
    test('language registrations cross the host boundary as generic events', async () => {
        const lithe = await launchInitialized();
        await lithe.request('host/activateByEvent', { event: 'onCommand:litheProbe.editAndSave' });
        await lithe.waitForNotification(entry => entry.method === 'lithe/languageProviderRegistered' && entry.params.kind === 'completion', 'completion provider registration');
        expect(lithe.notifications.some(entry => entry.method === 'lithe/languageProviderRegistered' && entry.params.kind === 'completion')).toBe(true);
        lithe.openDocument(lithe.uriOf('A.java'), 'class A {}\n', 'java');
        const registration = lithe.notifications.find(entry => entry.method === 'lithe/languageProviderRegistered' && entry.params.kind === 'completion')!;
        const result = await lithe.request('host/provideLanguageFeature', {
            kind: 'completion', handle: registration.params.handle, uri: lithe.uriOf('A.java'), line: 1, character: 7
        });
        expect(result.completions[0].label).toBe('litheProbe');
        const resolved = await lithe.request('host/resolveCompletion', { ...result.completions[0].data, uri: lithe.uriOf('A.java') });
        expect(resolved.detail).toBe('resolved by extension');
        expect(resolved.additionalTextEdits).toEqual([{
            range: { startLine: 1, startColumn: 1, endLine: 1, endColumn: 1 }, text: '// import from resolve\n'
        }]);
        const replacement = await lithe.request('host/provideLanguageFeature', {
            kind: 'completion', handle: registration.params.handle, uri: lithe.uriOf('A.java'), line: 1, character: 7
        });
        await expectHostError(lithe.request('host/resolveCompletion', { ...result.completions[0].data, uri: lithe.uriOf('A.java') }), 'invalidParams');
        lithe.notify('host/documentChanged', { uri: lithe.uriOf('A.java'), version: 2, changes: [], isDirty: true });
        await expectHostError(lithe.request('host/resolveCompletion', { ...replacement.completions[0].data, uri: lithe.uriOf('A.java') }), 'invalidParams');


        expect(result.completions[0].textEdit.range).toEqual({ startLine: 1, startColumn: 7, endLine: 1, endColumn: 8 });
        await lithe.request('host/executeCommand', { command: 'litheProbe.diagnostics', arguments: [{ $uri: lithe.uriOf('A.java') }] });
        await lithe.waitForNotification(entry => entry.method === 'lithe/diagnosticsChanged', 'diagnostics');
        const notification = lithe.notifications.find(entry => entry.method === 'lithe/diagnosticsChanged')!;
        expect(notification.params.delta[0][1][0]).toMatchObject({
            message: 'probe diagnostic', severity: 1,
            range: { startLine: 1, startColumn: 1, endLine: 1, endColumn: 6 }
        });
        await lithe.request('host/executeCommand', { command: 'litheProbe.clearDiagnostics', arguments: [] });
        await lithe.waitForNotification(entry => entry.method === 'lithe/diagnosticsCleared', 'clear diagnostics');

    }, TEST_TIMEOUT_MILLISECONDS);

    test('language results normalize hover and locations and honor each selector', async () => {
        const lithe = await launchInitialized();
        await lithe.request('host/activateByEvent', { event: 'onCommand:litheProbe.editAndSave' });
        await lithe.waitForNotification(entry => entry.method === 'lithe/languageProviderRegistered' && entry.params.kind === 'references', 'language registration');
        const uri = lithe.uriOf('A.java');
        lithe.openDocument(uri, 'class A {}\n', 'java');
        const registrations = lithe.notifications.filter(entry => entry.method === 'lithe/languageProviderRegistered');
        for (const kind of ['hover', 'definition', 'references']) {
            const handle = registrations.find(entry => entry.params.kind === kind)!.params.handle;
            const result = await lithe.request('host/provideLanguageFeature', { kind, handle, uri, line: 1, character: 7 });
            if (kind === 'hover') expect(result).toEqual({ contents: '**probe hover**', range: { startLine: 1, startColumn: 1, endLine: 1, endColumn: 6 } });
            else expect(result).toEqual([{ uri, range: { startLine: 1, startColumn: 7, endLine: 1, endColumn: 8 } }]);
        }
        const excluded = registrations.find(entry => entry.params.selector.some((selector: any) => selector.pattern === '**/Never.java'))!;
        expect(await lithe.request('host/provideLanguageFeature', {
            kind: 'completion', handle: excluded.params.handle, uri, line: 1, character: 7
        })).toEqual({ completions: [] });
        await expectHostError(lithe.request('host/provideLanguageFeature', {
            kind: 'completion', handle: -1, uri, line: 1, character: 7
        }), 'invalidParams');
    }, TEST_TIMEOUT_MILLISECONDS);

    test('completion identities expire on close and bounded cache eviction', async () => {
        const lithe = await launchInitialized();
        await lithe.request('host/activateByEvent', { event: 'onCommand:litheProbe.editAndSave' });
        await lithe.waitForNotification(entry => entry.method === 'lithe/languageProviderRegistered' && entry.params.kind === 'completion', 'completion registration');
        const handle = lithe.notifications.find(entry => entry.method === 'lithe/languageProviderRegistered' && entry.params.kind === 'completion')!.params.handle;
        let oldest: any;
        let newest: any;
        for (let index = 0; index < 33; index++) {
            const uri = lithe.uriOf(`A${index}.java`);
            lithe.openDocument(uri, 'class A {}\n', 'java');
            const result = await lithe.request('host/provideLanguageFeature', { kind: 'completion', handle, uri, line: 1, character: 7 });
            newest = { ...result.completions[0].data, uri };
            oldest ??= newest;
        }
        await expectHostError(lithe.request('host/resolveCompletion', oldest), 'invalidParams');
        expect((await lithe.request('host/resolveCompletion', newest)).detail).toBe('resolved by extension');
        lithe.notify('host/documentClosed', { uri: newest.uri });
        await expectHostError(lithe.request('host/resolveCompletion', newest), 'invalidParams');
    }, TEST_TIMEOUT_MILLISECONDS);
