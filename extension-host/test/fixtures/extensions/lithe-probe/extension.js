...
exports.activate = context => {
    const events = [];
    const diagnostics = vscode.languages.createDiagnosticCollection('litheProbe');
    const completion = vscode.languages.registerCompletionItemProvider('java', {
        provideCompletionItems: () => new vscode.CompletionList([new vscode.CompletionItem('litheProbe')]),
        resolveCompletionItem: item => {
            item.detail = 'resolved by extension';
            item.additionalTextEdits = [vscode.TextEdit.insert(new vscode.Position(0, 0), '// import from resolve\n')];
            return item;
        }
    });
    context.subscriptions.push(diagnostics, completion,
        vscode.languages.registerCompletionItemProvider({ language: 'java', pattern: '**/Never.java' }, {
            provideCompletionItems: () => [new vscode.CompletionItem('excluded')]
        }),
        vscode.languages.registerHoverProvider('java', {
            provideHover: () => new vscode.Hover(new vscode.MarkdownString('**probe hover**'), new vscode.Range(0, 0, 0, 5))
        }),
        vscode.languages.registerDefinitionProvider('java', {
            provideDefinition: document => new vscode.Location(document.uri, new vscode.Range(0, 6, 0, 7))
        }),
        vscode.languages.registerReferenceProvider('java', {
            provideReferences: document => [new vscode.Location(document.uri, new vscode.Range(0, 6, 0, 7))]
        })
    );
    context.subscriptions.push(vscode.commands.registerCommand('litheProbe.diagnostics', uri => {
        diagnostics.set(uri, [new vscode.Diagnostic(new vscode.Range(0, 0, 0, 5), 'probe diagnostic', vscode.DiagnosticSeverity.Error)]);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('litheProbe.clearDiagnostics', () => diagnostics.dispose()));
