...
async function createPackage(params: {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "lithe-db-extension-"));
...
const requestedBinDir = argValue("--bin-dir") || process.env.LITHE_DATABASE_SIDECAR_BIN_DIR;
const requestedBuildTargetDir =
  argValue("--target-dir") || process.env.LITHE_DATABASE_SIDECAR_TARGET_DIR;
...
const temporaryBuildTargetDir =
  shouldBuild && !requestedBuildTargetDir
    ? await mkdtemp(join(tmpdir(), "lithe-db-sidecars-"))
    : undefined;
const buildTargetDir = shouldBuild
  ? requestedBuildTargetDir
    ? resolve(LITHE_ROOT, requestedBuildTargetDir)
    : temporaryBuildTargetDir
  : undefined;
const binDir = buildTargetDir
  ? join(buildTargetDir, "release")
  : resolve(LITHE_ROOT, requestedBinDir || "target/release");
...
async function buildSidecar(providerId: string, binaryName: string) {
  ...
  await $`cargo build -p lithe-database --release --no-default-features --features ${providerId} --bin ${binaryName} --target-dir ${buildTargetDir}`.cwd(
    LITHE_ROOT,
  );
}
...
try {
  for (const { folder, manifest, provider } of await findDatabaseExtensionFolders(providerFilter)) {
    ...
    if (
      ...
      throw new Error(
        `Missing database sidecar binary for ${providerId}: ${binaryPath}. Run this script with --build, or build it from the Lithe repo with: cargo build -p lithe-database --release --no-default-features --features ${providerId} --bin ${basename(sidecarPath)}`,
      );
    }
