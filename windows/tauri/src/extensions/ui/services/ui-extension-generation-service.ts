...
export async function requestUIExtensionGeneration(params: {
  ...
  if (!token) {
    throw new UIExtensionGenerationError("Sign in to Lithe to use hosted UI generation.", 401);
  }
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      message = "Sign in to Lithe to use hosted UI generation.";
    } else if (response.status === 403) {
      message = "Lithe Pro is required to generate hosted UI extensions.";
    }
