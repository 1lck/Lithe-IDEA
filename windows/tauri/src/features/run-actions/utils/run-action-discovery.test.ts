...
describe("PHP run actions", () => {
  ...
  test("discovers a phpunit action for a PHPUnit project", async () => {
    ...
    const actions = await discoverProjectRunActions(
      "/work/api",
      async (path) => {
...
test("disabled PHP does not read PHP manifests or expose actions", async () => {
  const reads: string[] = [];
  ...
  expect(actions.some((action) => action.source === "php")).toBe(false);
});

test("Composer arrays retain the named script and structured launch arguments", () => {
  const actions = parseComposerRunActions(
    JSON.stringify({ scripts: { test: ["@php vendor/bin/phpunit", "@lint"] } }),
    "/work/api",
  );
  expect(actions).toHaveLength(1);
  expect(actions[0]?.pluginCommand).toEqual({
    executable: "composer",
    arguments: ["run", "--", "test"],
  });
});
