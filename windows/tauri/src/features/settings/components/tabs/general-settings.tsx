  ...
  return (
    <SettingsView>
      ...
      <div className="font-sans ui-text-sm -mt-3 px-1 text-subtle-foreground/75">
        {downloading
          ? `Lithe ${appVersion || "..."} · Downloading ${downloadProgress?.percentage ?? 0}%`
          : installing
            ? `Lithe ${appVersion || "..."} · Installing update...`
            : available
              ? `Lithe ${appVersion || "..."} · Version ${updateInfo?.version} available`
              : error
                ? `Lithe ${appVersion || "..."} · Failed to check for updates`
                : `Lithe ${appVersion || "..."} · App is up to date`}
      </div>

      {downloading && downloadProgress ? (
        <Progress
          value={downloadProgress.percentage}
          aria-label="Lithe update download progress"
          className="px-3"
        />
      ) : null}
      ...
      <SettingRow
        ...
        description="Install the `lithe` command to open folders and files from your terminal."
      >
      ...
      <div className="font-sans ui-text-sm -mt-3 px-1 text-subtle-foreground/75">
        {cliChecking
          ? "Checking..."
          : cliInstalled
            ? "CLI command is installed at $HOME/.local/bin/lithe"
