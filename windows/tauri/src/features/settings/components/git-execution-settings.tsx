...
export function GitExecutionSettings() {
  ...
  useEffect(() => {
    ...
    setSnapshot(null); setError(""); setSaved(false); setBusy(advancedOpen && !!root);
    if (advancedOpen && root) void invoke<Snapshot>("git.executionInspect", { root, scope }).then((result) => {
  ...
  }, [advancedOpen, root, scope, executable, helper, fetchPrune, fetchSubmodules, fetchTags, reload]);
  ...
  return <Section title={t("git.execution.title")}>
    ...
    <details className="space-y-3" open={advancedOpen} onToggle={(event) => setAdvancedOpen(event.currentTarget.open)}>
      <summary className="cursor-pointer text-sm font-medium">{t("git.execution.advanced")}</summary>
