...
export function GitLogToolWindow() {
  ...
  const copyCommitText = async (text: string, label: string) => {
    ...
    toast.error(`Unable to copy ${label.toLocaleLowerCase()}`);
  };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background text-foreground">
      <GitLogTitleBar
        referenceName={selectedReference?.shortName ?? "All"}
        ...
        onCopyHash={() => {
          if (selectedCommit) void copyCommitText(selectedCommit.hash, "Commit hash");
        }}
      ...
      {loadState === "failed" && history.commits.length > 0 ? (
        <div className="flex h-7 shrink-0 items-center gap-2 border-destructive/30 border-b bg-destructive/10 px-2 font-sans ui-text-sm text-destructive">
          <span className="min-w-0 flex-1 truncate">{error ?? "Unable to refresh Git log."}</span>
          <button
            type="button"
      ...
      {!repoPath ? (
        <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-2 text-subtle-foreground">
          <div className="font-medium text-foreground">No repository open</div>
          <div>Open a Git workspace to view its log.</div>
        </div>
        ...
        <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-3 px-6 text-center">
          <div className="text-destructive">{error ?? "Unable to load Git log."}</div>
        ...
        >
          ...
          <ResizablePanel id="commits" defaultSize="57" minSize={320}>
            <GitCommitTable
              ...
              onCopyHash={(commit) => void copyCommitText(commit.hash, "Commit hash")}
