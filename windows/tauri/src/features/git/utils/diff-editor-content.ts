...
const DIFF_ACCORDION_PREFIX = "\uE000LITHE_DIFF_FILE ";
...
export function serializeGitDiffForEditor(diff: GitDiff): string {
  if (diff.raw_patch) {
    ...
    return [
      ...
      `# Lithe truncated this diff after ${DIFF_SERIALIZED_LINE_LIMIT.toLocaleString()} lines to keep the editor responsive.`,
    ].join("\n");
  }
