...
const AUTH_API_BASE_STORAGE_KEY = "lithe_auth_api_base";
...
const COLLABORATION_DEVICE_ID_STORAGE_KEY = "lithe_collaboration_device_id";
const COLLABORATION_CLIENT_SEQ_STORAGE_KEY = "lithe_collaboration_client_seq";
...
export const getAuthToken = async (): Promise<string | null> => {
  ...
  try {
    authTokenCache = await invoke<string | null>("get_secure_secret", { key: "auth-token" });
    return authTokenCache;
  } catch {
    return null;
  }
...
export const storeAuthToken = async (token: string): Promise<void> => {
  await invoke("store_secure_secret", { key: "auth-token", value: token });
...
export const removeAuthToken = async (): Promise<void> => {
  authTokenCache = null;
  clearAuthApiBase();
  await invoke("remove_secure_secret", { key: "auth-token" });
};
