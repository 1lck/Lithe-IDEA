...
const LITHE_OUTPUT_START_PATTERN = /^\s*<!--\s*lithe:r-output:start\b.*-->\s*$/;
const LITHE_OUTPUT_END_PATTERN = /^\s*<!--\s*lithe:r-output:end\s*-->\s*$/;
...
function findOutputRange(
  ...
  for (let line = closeLine + 1; line < lines.length; line += 1) {
    ...
    if (!LITHE_OUTPUT_START_PATTERN.test(lines[line])) return null;

    for (let endLine = line + 1; endLine < lines.length; endLine += 1) {
      if (LITHE_OUTPUT_END_PATTERN.test(lines[endLine])) {
        return {
