...
const STORAGE_KEY = "lithe:file-open-benchmark";
...
export const fileOpenBenchmark = {
  ...
  finish(path: string, label = "done", detail?: string, meta: FileOpenBenchmarkMeta = {}): void {
    ...
    console.info(
      `[lithe:file-open] file=${shortPath(path)} type=${fileType} lines=${meta.lineCount ?? "unknown"} totalMs=${summary.total.toFixed(1)} seconds=${seconds.toFixed(3)} chars=${meta.contentLength ?? "unknown"} large=${meta.largeContentMode ?? "unknown"}`,
    );
