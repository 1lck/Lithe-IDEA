  ...
  const renderEndpointRow = useCallback(
    (endpoint: SpringEndpoint) => (
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate font-sans ui-text-sm text-subtle-foreground">
          {endpoint.controller}.{endpoint.method}
        </span>
        <span className="min-w-0 flex-[1.2] truncate font-mono text-[11px] text-subtle-foreground">
          {endpoint.path}
        </span>
      </button>
    ),
