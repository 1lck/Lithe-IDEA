...
export const createAdvancedActions = (params: AdvancedActionsParams): Action[] => {
  ...
  const baseActions: Action[] = [
    ...
    {
      id: "developer-open-lithe-log",
      label: "Developer: Open Lithe Log",
      description: "Open the current Lithe application log in a read-only editor tab",
      icon: <Terminal />,
      category: "Developer",
      action: async () => {
        try {
          ...
          showToast({
            message: error instanceof Error ? error.message : "Failed to open Lithe log",
            type: "error",
          });
        }
        onClose();
    ...
    },
    {
      ...
      description: "Install 'lithe' command for terminal",
      icon: <Terminal />,
      category: "CLI",
      action: async () => {
        try {
