    ...
    <form id={formId} className="space-y-4" onSubmit={handleSubmit}>
      ...
      <Field>
        <FieldLabel htmlFor={`${idPrefix}-name`}>
          {t("remote.connectionName")} <span className="text-subtle-foreground">*</span>
        </FieldLabel>
        <Input
          ref={nameInputRef}
          id={`${idPrefix}-name`}
          ...
          placeholder={t("remote.connectionNamePlaceholder")}
          size="md"
          disabled={disabled}
        />
      </Field>
      ...
      <div className="grid grid-cols-12 gap-3">
        <Field className="col-span-8">
          <FieldLabel htmlFor={`${idPrefix}-host`}>
            {t("database.host")} <span className="text-subtle-foreground">*</span>
          </FieldLabel>
          <Input
            id={`${idPrefix}-host`}
            type="text"
        ...
        <Field className="col-span-4">
          <FieldLabel htmlFor={`${idPrefix}-port`}>{t("database.port")}</FieldLabel>
          <Input
            id={`${idPrefix}-port`}
            type="number"
            value={formData.port}
      ...
      <Field>
        <FieldLabel htmlFor={`${idPrefix}-type`}>{t("remote.connectionType")}</FieldLabel>
        <Select
          id={`${idPrefix}-type`}
          value={formData.type}
      ...
      <Field>
        <FieldLabel htmlFor={`${idPrefix}-username`}>
          {t("database.username")} <span className="text-subtle-foreground">*</span>
        </FieldLabel>
        <Input
      ...
      <Field>
        <FieldLabel htmlFor={`${idPrefix}-password`}>
          {t("database.password")}{" "}
          <span className="text-subtle-foreground">({t("database.optional")})</span>
        </FieldLabel>
        <InputGroup>
          <InputGroupInput
            ...
            placeholder={t("remote.leaveEmptyForKeyAuth")}
            size="md"
            disabled={disabled}
          />
          <InputGroupAddon align="inline-end">
            <InputGroupButton
              ...
              aria-label={showPassword ? t("remote.hidePassword") : t("remote.showPassword")}
      ...
      {formData.password ? (
        <Field orientation="horizontal">
          ...
          <FieldLabel htmlFor={`${idPrefix}-save-credentials`} className="cursor-pointer">
            {t("remote.savePasswordForFutureConnections")}
          </FieldLabel>
        </Field>
      ) : null}
      ...
      {validationStatus === "valid" ? (
        <Marker tone="success">
          ...
          <MarkerContent>{t("remote.connectionSavedSuccessfully")}</MarkerContent>
        </Marker>
      ) : null}
