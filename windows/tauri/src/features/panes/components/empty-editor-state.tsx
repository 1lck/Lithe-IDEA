      ...
      <ContextMenuTrigger className="flex h-full min-h-0 w-full overflow-auto">
        <Empty className="m-auto max-w-2xl gap-4 px-6 py-8">
          ...
          {isAgentAvailable ? (
            ...
            >
              <div className="w-full opacity-50 [&_*]:pointer-events-none" aria-disabled="true">
                <AgentLaunchInput active={false} autoFocus={false} surfaceId="empty-editor" />
              </div>
            </Tooltip>
          )}
          ...
          <div className="grid w-full grid-cols-[repeat(auto-fit,minmax(8rem,1fr))] gap-2">
            {quickActions.map((item) => (
              <Button
                ...
                tooltip={item.disabled ? item.tooltip : undefined}
                tooltipSide="top"
