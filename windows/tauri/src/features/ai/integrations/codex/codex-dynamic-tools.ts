export const LITHE_OPEN_PULL_REQUEST_TOOL = "lithe_open_pull_request";
export const LITHE_OPEN_ISSUE_TOOL = "lithe_open_issue";
export const LITHE_SET_CHAT_TITLE_TOOL = "lithe_set_chat_title";
...
export function runCodexDynamicTool(
  ...
  if (tool === LITHE_SET_CHAT_TITLE_TOOL) {
    ...
    if (!options.setChatTitle(title.slice(0, 80))) {
      return toolResult("The Lithe chat is no longer available.", false);
    }
    return toolResult(`Lithe chat renamed to "${title.slice(0, 80)}".`, true);
  }

  const isPullRequest = tool === LITHE_OPEN_PULL_REQUEST_TOOL;
  const isIssue = tool === LITHE_OPEN_ISSUE_TOOL;
  ...
  return toolResult(`${label} #${resourceNumber} opened in Lithe.`, true);
}
