...
export const Tab = forwardRef<HTMLDivElement, TabProps>(function Tab(
  ...
  return (
    <div
      ...
      {...props}
    >
      <div className="flex min-w-0 flex-1 items-center gap-(--lithe-chrome-gap-loose)">
        {children}
      </div>
      {action}
    </div>
...
const tabBarTabVariants = cva("ui-text-chrome", {
  variants: {
    orientation: {
      horizontal: "h-(--lithe-tab-height) min-w-20 max-w-(--lithe-tab-max-width) w-fit pl-2 pr-6",
      vertical: "min-h-(--lithe-tab-height) w-full max-w-none justify-start rounded-md pl-2 pr-6",
    },
  },
  defaultVariants: {
    orientation: "horizontal",
