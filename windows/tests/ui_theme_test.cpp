...
int main(int argc, char* argv[]) {
    ...
    assert(style.contains(QStringLiteral("#202124")));
    ...
    assert(style.contains(QStringLiteral("QWidget#workbench\\.editorArea { background: #1e1f22; border: 0; }")));
    assert(!style.contains(QStringLiteral("border-radius: 8px")));
    assert(application.palette().color(QPalette::Highlight) == QColor(QStringLiteral("#2f506e")));
    return 0;
}
