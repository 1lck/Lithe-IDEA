...
From a Windows GUI host, subprocesses launched by lithe-core that are console
subsystem programs (such as git) must not flash an independent console window.
Because the app process has no console, a CUI child writing to stdout/stderr
would otherwise trigger an implicit `AllocConsole` and pop a window titled with
the child's executable path.
