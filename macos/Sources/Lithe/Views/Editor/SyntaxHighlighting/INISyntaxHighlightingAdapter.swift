...
/// Highlights INI sections, assignments, scalar values, and line comments.
enum INISyntaxHighlightingAdapter {
    ...
    private static let booleanExpression = expression(
        #"(?<![A-Za-z0-9_.-])(?:true|false|yes|no|on|off)(?![A-Za-z0-9_.-])"#,
    ...
    static func apply(to storage: NSTextStorage, palette: SyntaxHighlightingPalette, range target: NSRange) {
        ...
        while lineStart < scanLimit {
            ...
            if let sectionRange = sectionNameRange(in: source, range: bodyRange) {
    ...
    private static func apply(
        ...
        range: NSRange,
        excluding quotedRanges: [NSRange],
        target: NSRange
    ) {
        guard range.length > 0 else { return }
        expression.enumerateMatches(in: source as String, range: range) { match, _, _ in
            guard let match,
                  !quotedRanges.contains(where: { NSIntersectionRange(match.range, $0).length > 0 }) else {
                return
            }
    ...
    private static func sectionNameRange(in source: NSString, range: NSRange) -> NSRange? {
    ...
    private static func firstAssignmentSeparator(in source: NSString, range: NSRange) -> Int? {
        ...
        while cursor < NSMaxRange(range) {
            ...
            } else if (character == 61 || character == 58), !inSingleQuote, !inDoubleQuote {
                return cursor
            }
            cursor += 1
        }
    ...
    private static func firstCommentLocation(in source: NSString, range: NSRange) -> Int? {
        ...
        while cursor < NSMaxRange(range) {
            ...
            if character == 34, !inSingleQuote {
                ...
                inSingleQuote.toggle()
            } else if (character == 35 || character == 59),
                      !inSingleQuote,
                      !inDoubleQuote,
    ...
    private static func quotedStringRanges(in source: NSString, range: NSRange) -> [NSRange] {
        ...
        while cursor < NSMaxRange(range) {
            ...
            while cursor < NSMaxRange(range) {
                let character = source.character(at: cursor)
                if character == 92, quote == 34, cursor + 1 < NSMaxRange(range) {
                    cursor += 2
                    continue
                }
                cursor += 1
