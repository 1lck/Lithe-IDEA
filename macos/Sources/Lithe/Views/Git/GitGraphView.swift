...
final class GitGraphScrollDocumentView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        guard canLoadMore, dirtyRect.maxY >= footerY else { return }
        let isDark = effectiveAppearance.bestMatch(from: [.aqua, .darkAqua]) == .darkAqua
        let primary = LitheTheme.nsColor(.accent, isDark: isDark)
        ...
        let title = isLoadingMore ? String(localized: "Loading commits…") : String(localized: "Load more commits")
        let font = NSFont.systemFont(ofSize: 11.5, weight: .medium)
        let attributes: [NSAttributedString.Key: Any] = [.font: font, .foregroundColor: primary]
        let textSize = (title as NSString).size(withAttributes: attributes)
        (title as NSString).draw(
            in: CGRect(
                x: (bounds.width - textSize.width) / 2,
                y: footerY + (rowHeight - textSize.height) / 2,
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        let footerY = CGFloat(presentationRowCount) * rowHeight + (hasMissingParents ? rowHeight : 0)
        if canLoadMore, point.y >= footerY, !isLoadingMore {
            onLoadMore?()
            return
        }
