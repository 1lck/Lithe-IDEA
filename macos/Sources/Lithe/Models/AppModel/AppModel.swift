...
final class AppModel: ObservableObject, Identifiable {
    ...
    init(settings: AppSettings, services: AppServices) {
        ...
        workspaceFeature.configureProjection(
            ...
            onSnapshotLoaded: { [weak self] workspaceURL, snapshot, isInitialLoad in
                ...
                // The rebuild already rejected a stale workspace and carries its
                // URL with the snapshot, so the file list and identity stay paired.
    ...
    /// Records teardown synchronously so a following on-demand activation can
    /// join it before the runtime releases the newly activated capability.
    ...
    /// Waits for every teardown already registered for this session.
