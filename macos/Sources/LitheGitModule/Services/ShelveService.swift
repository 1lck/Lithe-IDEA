...
package struct ShelveService: Sendable {
    private static let formatVersion = 1
    private let storage: any GitShelfStorage

    package init(storage: any GitShelfStorage) {
    ...
    private static func readEntries(
        ...
        return storage.listDirectory(at: directory)
            ...
            .compactMap { url in
                guard let data = try? storage.readData(from: url) else { return nil }
    ...
    private static func write(
        ...
        do {
            ...
            try storage.writeData(data, to: url)
            return true
        } catch {
            return false
        }
