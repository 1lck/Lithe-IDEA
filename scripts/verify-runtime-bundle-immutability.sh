...
bundle_write_pattern='(Bundle\.main\.(resourceURL|bundleURL)|Contents/Resources)[^\n]*(write|createDirectory|copyItem|moveItem|removeItem)|((write|createDirectory|copyItem|moveItem|removeItem)[^\n]*)(Bundle\.main\.(resourceURL|bundleURL)|Contents/Resources)'
bundle_write_violations=$(
    rg --pcre2 -n --glob '*.swift' "$bundle_write_pattern" macos/Sources || true
)
...
for required in \
    ...
    if ! rg -Fq -- "$required" "$resolver"; then
...
if ! rg -Fq -- 'configurationCacheDirectoryURL: languageServerCacheDirectory' "$container"; then
