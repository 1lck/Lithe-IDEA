...
macOS CI 与发布工作流保持 Swift 6.2、`setup-swift@v2` 和一致的依赖缓存版本。
本机可以使用更新的 Xcode，但工具链升级必须单独验证安装、编译、测试和双架构
打包后再一起切换。不能为了修一个编译错误，同时把所有 CI 和发布任务迁到
尚未验证的安装路径；否则安装器崩溃会让产品测试根本无法开始。
