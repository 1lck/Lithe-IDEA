...
enum SyntaxHighlighter {
    ...
    static func applyExact(
        ...
        switch SyntaxHighlightingRegistry.bundled.adapter(
            fileName: fileName,
            fileExtension: fileExtension
        ) {
        case .generic:
...
enum SyntaxHighlightingAdapterSupport {
    static func apply(
        ...
        expression.enumerateMatches(in: storage.string, range: range) { match, _, _ in
            ...
            storage.addAttribute(.foregroundColor, value: color, range: matchedRange)
        }
    }
}
