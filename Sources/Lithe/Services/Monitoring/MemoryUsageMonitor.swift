...
final class ManagedProcessRegistry: @unchecked Sendable {
    ...
    private var moduleEntries: [ModuleID: Set<Int32>] = [:]

    func register(pid: Int32, category: ManagedProcessCategory) {
        register(pid: pid, category: category, moduleID: nil)
    }

    func register(pid: Int32, category: ManagedProcessCategory, moduleID: ModuleID?) {
        guard pid > 0 else { return }
        lock.lock(); defer { lock.unlock() }
        entries[category, default: []].insert(pid)
        if let moduleID { moduleEntries[moduleID, default: []].insert(pid) }
    }

    func unregister(pid: Int32, category: ManagedProcessCategory) {
        unregister(pid: pid, category: category, moduleID: nil)
    }

    func unregister(pid: Int32, category: ManagedProcessCategory, moduleID: ModuleID?) {
        lock.lock(); defer { lock.unlock() }
        entries[category]?.remove(pid)
        if let moduleID {
            moduleEntries[moduleID]?.remove(pid)
        } else {
            for id in moduleEntries.keys { moduleEntries[id]?.remove(pid) }
        }
    }
    ...
    func processIDs(for moduleID: ModuleID) -> Set<Int32> {
        lock.lock(); defer { lock.unlock() }
        return moduleEntries[moduleID] ?? []
    }

    func processCount(for moduleID: ModuleID) -> Int {
        processIDs(for: moduleID).count
    }
}
