...
package struct DatabaseDBXImportFolder: Equatable, Identifiable, Sendable {
    package let id: UUID
    package let name: String
    package let parentID: UUID?
}

package struct DatabaseDBXImportCandidate: Equatable, Identifiable, Sendable {
    package let sourceID: String
    package var profile: DatabaseProfile
    package let password: String
    package let warnings: [String]
    package let isDuplicate: Bool

    package var id: UUID { profile.id }
}

package struct DatabaseDBXImportPlan: Equatable, Sendable {
    package let candidates: [DatabaseDBXImportCandidate]
    package let folders: [DatabaseDBXImportFolder]
    package let unsupportedTypes: [String: Int]
    package let wasEncrypted: Bool

    package var importableCount: Int { candidates.count { !$0.isDuplicate } }
    package var duplicateCount: Int { candidates.count { $0.isDuplicate } }
    package var unsupportedCount: Int { unsupportedTypes.values.reduce(0, +) }
}

package enum DatabaseDBXImportError: LocalizedError, Equatable {
    case invalidFile
...
package struct DatabaseDBXImportService: Sendable {
    package init() {}
    package func isEncrypted(_ data: Data) -> Bool {
...
private struct DBXEncryptedEnvelope: Decodable {
    ...
    package let iv: String
    package let data: String
}

private struct DBXExport: Decodable {
    package let connections: [DBXConnection]
    package let layout: DBXLayout?
}

private struct DBXConnection: Decodable {
    package let id: String
    package let name: String
    package let dbType: String
    ...
    package let isProduction: Bool?
    package let ssl: Bool?
    package let caCertPath: String?
    package let clientCertPath: String?
    package let clientKeyPath: String?
    package let connectionString: String?
    package let urlParams: String?
    package let transportLayers: [DBXTransportLayer]?
    package let redisConnectionMode: String?
...
private struct DBXLayout: Decodable {
    package let groups: [DBXGroup]
    package let order: [DBXOrderEntry]
}
...
private struct DBXOrderEntry: Decodable {
    package let type: String
    package let id: String
    package let children: [DBXOrderEntry]?
    package let connectionIds: [String]?
}

private struct DBXResolvedLayout {
    package let folders: [DatabaseDBXImportFolder]
    package let connectionFolderIDs: [String: UUID]
}
