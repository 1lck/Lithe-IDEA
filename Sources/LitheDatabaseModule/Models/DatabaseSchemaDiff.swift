...
package struct DatabaseSchemaColumnSnapshot: Codable, Equatable, Sendable {
    package let name: String
    package let dataType: String
    package let isNullable: Bool
    package let defaultValue: String?
    package let isPrimaryKey: Bool
}

package struct DatabaseSchemaIndexSnapshot: Codable, Equatable, Sendable {
    package let name: String
    package let definition: String
}

package struct DatabaseSchemaForeignKeySnapshot: Codable, Equatable, Sendable {
    package let name: String
    package let column: String
    package let referencedTable: String
    package let referencedColumn: String
}

package struct DatabaseSchemaTableSnapshot: Codable, Equatable, Sendable {
    package let name: String
    package let columns: [DatabaseSchemaColumnSnapshot]
    package let indexes: [DatabaseSchemaIndexSnapshot]
    package let foreignKeys: [DatabaseSchemaForeignKeySnapshot]
}

package struct DatabaseSchemaSnapshot: Codable, Equatable, Sendable {
    package let profileID: UUID
    package let profileName: String
    package let kind: DatabaseKind
    package let schema: String
    package let tables: [DatabaseSchemaTableSnapshot]
}

private struct DatabaseForeignKeyGroup {
    package let name: String
    package let referencedTable: String
    package let columns: [DatabaseSchemaForeignKeySnapshot]
}

package enum DatabaseSchemaDiffKind: String, Codable, CaseIterable, Sendable {
    case addTable
    case dropTable
    case addColumn
    case dropColumn
...
package struct DatabaseSchemaDiffItem: Codable, Equatable, Identifiable, Sendable {
    package let id: String
    package let kind: DatabaseSchemaDiffKind
    package let table: String
    package let detail: String
    package let sql: String

    package var isDestructive: Bool { kind.isDestructive }
}

package struct DatabaseSchemaDiffResult: Codable, Equatable, Sendable {
    package let source: DatabaseSchemaSnapshot
    package let target: DatabaseSchemaSnapshot
    package let items: [DatabaseSchemaDiffItem]

    package var requiresConfirmation: Bool { items.contains(where: { $0.isDestructive }) }
    package var migrationSQL: String { items.map(\.sql).joined(separator: "\n") }
}

package enum DatabaseSchemaDiffEngine {
    package static func statements(in sql: String) -> [String] {
        var result: [String] = []
        var statement = ""
        var quote: Character?
        var escaped = false
    ...
    package static func compare(source: DatabaseSchemaSnapshot, target: DatabaseSchemaSnapshot) -> DatabaseSchemaDiffResult {
