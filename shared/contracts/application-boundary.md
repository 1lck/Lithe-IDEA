...
| Agent conversation (ACP) | ACP v1 connection per workspace, API-key gateway sign-in, agent-owned session history (list/load), streamed updates, permission decisions, cancellation, and process-tree lifecycle in the shared Rust host | executable and provider selection, API-key storage, workspace selection, module enablement, and conversation UI |
