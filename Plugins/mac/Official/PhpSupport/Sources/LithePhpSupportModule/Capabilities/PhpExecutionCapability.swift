...
public final class PhpExecutionCapability: NSObject,
    ...
    public func launchPlan(
        ...
        let path = request.relativeFilePath
        guard !path.isEmpty,
              !path.hasPrefix("/"),
        ...
        return LanguageRunExtensionPlan(
            ...
            arguments: [Self.commandPath(path)] + request.arguments,
    ...
    public func testPlan(
        ...
        case .file(let relativePath):
            ...
            arguments = [Self.phpUnitEntryPoint, Self.commandPath(path)]
        ...
        case .testCase(let identifier, let relativeFilePath):
            ...
            if let relativeFilePath {
                caseArguments.append(Self.commandPath(try Self.checkedRelativePath(relativeFilePath)))
            }
    ...
    private static func commandPath(_ path: String) -> String {
