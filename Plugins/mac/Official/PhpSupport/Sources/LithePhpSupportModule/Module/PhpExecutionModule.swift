...
public final class PhpExecutionModule: LitheModule {
    ...
    private var capability: PhpExecutionCapability?
    ...
    public func activate(context: ModuleContext) async throws {
        guard let executionHost else {
            throw LanguageExtensionHostError.missingExecutionHost(languageID: phpLanguageID)
        }
        let sessions = PhpExecutionResource(
        ...
        capability = PhpExecutionCapability(sessionFactory: { sessions.makeSession() })
    }
    ...
    public func exportedCapabilities() -> [ModuleCapabilityID: AnyObject] {
        capability.map {
            [
                .languageExecutionExtension(phpLanguageID): $0,
                .languageTestingExtension(phpLanguageID): $0
            ]
        } ?? [:]
    }
...
@MainActor
private final class PhpExecutionResource: ModuleResource {
    ...
    func makeSession() -> any LanguageExecutionSession {
        let session = PhpOwnedExecutionSession(
...
@MainActor
private final class PhpOwnedExecutionSession: LanguageExecutionSession {
