...
pub fn menu_text(cx: &App, key: &str) -> &'static str {
    let (zh, en) = match key {
        ...
        "run.title" => ("运行", "Run"),
        "run.running" => ("运行中", "Running"),
        "run.run" => ("运行配置", "Run configuration"),
        "run.stop" => ("停止运行", "Stop run"),
        "run.rescan" => ("重新扫描服务", "Rescan services"),
        "run.clearOutput" => ("清除运行输出", "Clear run output"),
        "run.minimize" => ("最小化", "Minimize"),
        "run.emptyOutput" => (
            "运行配置后将在这里显示进程输出。",
            "Run a configuration to see process output.",
        ),
        "run.starting" => ("开始运行", "Starting"),
        "run.noSteps" => ("无可执行步骤", "No executable steps"),
        "run.startFailed" => ("启动失败", "Failed to start"),
        "run.exitCode" => ("退出码", "Exit code"),
        "run.exited" => ("进程已退出", "Process exited"),
        "run.finished" => ("运行已结束", "Run finished"),
        "run.stopping" => ("已发送停止信号…", "Stop signal sent…"),
        "maven.clearOutput" => ("清除构建输出", "Clear build output"),
        "maven.rerunTest" => ("重新运行测试", "Rerun test"),
        ...
        "terminal.sessionExited" => ("会话已退出", "Session exited"),
        "terminal.exitedCode" => (
            "[lithe] 终端进程已退出（退出码 {detail}）。",
            "[lithe] The terminal process exited with code {detail}.",
        ),
        "terminal.signalled" => (
            "[lithe] 终端进程被信号 {detail} 终止。",
            "[lithe] The terminal process was terminated by signal {detail}.",
        ),
        "terminal.exitUnknown" => (
            "[lithe] 终端会话已关闭。",
            "[lithe] The terminal session was closed.",
        ),
        "terminal.searchPlaceholder" => ("在终端输出中查找…", "Find in terminal output..."),
        "terminal.find" => ("在终端中查找", "Find in Terminal"),
        "terminal.searchPrevious" => ("上一个匹配", "Previous match"),
        "terminal.searchNext" => ("下一个匹配", "Next match"),
        "terminal.searchClose" => ("关闭搜索", "Close search"),
        "terminal.restartSession" => ("重启终端会话", "Restart terminal session"),
        "terminal.scrollToBottom" => ("回到底部", "Scroll to bottom"),
        ...
        // ---- 全局搜索（键与中英文逐条抄自 `windows/tauri/src/i18n/locale.ts`）----
        "search.emptyTitle" => ("在项目中搜索", "Search across your project"),
        "search.emptyDescription" => (
            "输入关键词，即可在整个项目中查找匹配的文件和代码行。",
            "Type to find matching files and code lines across your project.",
        ),
        "search.openProjectTitle" => ("打开项目后再搜索", "Open a project to search"),
        "search.openProjectDescription" => (
            "全局搜索需要先打开一个项目文件夹。",
            "Global search needs an open project folder.",
        ),
        "search.unsupported" => (
            "当前工作区类型不支持全局搜索。",
        ...
        "search.retry" => ("重试", "Try again"),
        "search.noResults" => ("未找到结果", "No results found"),
        "search.noResultsFor" => ("未找到 “{query}” 的结果", "No results found for \"{query}\""),
        "search.noResultsForWithFilters" => (
            "未找到 “{query}” 的结果（已应用当前文件筛选）",
            "No results found for \"{query}\" with the current file filters",
        ),
        "search.matchCase" => ("区分大小写", "Match case"),
        "search.matchWholeWord" => ("全字匹配", "Match whole word"),
        "search.useRegex" => ("使用正则表达式", "Use regular expression"),
        "search.search" => ("搜索", "Search"),
        "search.hideDetails" => ("隐藏详细信息", "Hide details"),
        "search.showDetails" => ("显示详细信息", "Show details"),
        "search.clear" => ("清除搜索", "Clear search"),
        "search.close" => ("关闭搜索", "Close search"),
        "search.previousMatch" => ("上一个匹配项", "Previous match"),
        "search.nextMatch" => ("下一个匹配项", "Next match"),
        "search.hideReplace" => ("隐藏替换", "Hide replace"),
        "search.showReplace" => ("显示替换", "Show replace"),
        "search.replaceWith" => ("替换为...", "Replace with..."),
        "search.replace" => ("替换", "Replace"),
        "search.replaceAllShort" => ("全部", "All"),
        "search.replacedMatches" => ("已替换 {count} 处匹配", "Replaced {count} matches"),
        "search.replaceMatchesFailed" => (
            "替换搜索匹配失败",
            "Failed to replace search matches",
        ),
        "search.options" => ("搜索选项", "Search options"),
        "search.filesToInclude" => ("要包含的文件", "Files to include"),
        "search.filesToExclude" => ("要排除的文件", "Files to exclude"),
        ...
        "maven.runSelected" => ("运行选中的生命周期阶段", "Run selected lifecycle phase"),
        "maven.stop" => ("停止 Maven 任务", "Stop Maven task"),
        "maven.executeGoal" => ("执行 Maven Goal", "Execute Maven goal"),
        "maven.skipTests" => ("跳过测试", "Skip tests"),
        "maven.collapseAll" => ("全部折叠", "Collapse all"),
        "maven.settings" => ("Maven 设置", "Maven Settings"),
        "maven.profiles" => ("Profiles", "Profiles"),
        "maven.dependencies" => ("依赖项", "Dependencies"),
        "maven.sourceRoots" => ("源码根目录", "Source Roots"),
        "maven.dependencyLoading" => ("正在解析依赖项...", "Resolving dependencies..."),
        "maven.dependencyCancelled" => ("依赖项解析已取消", "Dependency resolution cancelled"),
        "maven.noDependencies" => ("没有依赖项", "No dependencies"),
        "maven.omittedConflict" => ("版本冲突，采用", "conflict with"),
        "maven.omittedDuplicate" => ("已省略重复依赖", "duplicate omitted"),
        "maven.project" => ("项目", "Project"),
        "maven.loadFailed" => ("无法加载 Maven 项目", "Unable to load Maven project"),
        "maven.scanning" => ("正在扫描 Maven 项目...", "Scanning Maven project..."),
        "maven.notDetected" => ("未检测到 Maven 项目", "No Maven project detected"),
        "ui.retry" => ("重试", "Retry"),
        "ui.cancel" => ("取消", "Cancel"),
        // ---- 通用 UI ----
        "ui.close" => ("关闭", "Close"),
        ...
        "git.loadingWorktrees" => ("正在加载工作树...", "Loading worktrees..."),
        "git.noMatchingWorktrees" => ("没有匹配的工作树", "No matching worktrees"),
        "git.noWorktreesFound" => ("未找到工作树", "No worktrees found"),
        "git.createWorktree" => ("创建工作树 “{path}”", "Create worktree \"{path}\""),
        "git.detectingRepositories" => ("正在检测仓库...", "Detecting repositories..."),
        "git.noMatchingRepositories" => ("没有匹配的仓库", "No matching repositories"),
        "git.noRepositoriesFound" => ("未找到仓库", "No repositories found"),
        "git.newBranch" => ("新建分支", "New Branch"),
        "git.refresh" => ("刷新", "Refresh"),
        "git.add" => ("添加", "Add"),
        "git.adding" => ("正在添加...", "Adding..."),
        "git.clearAdded" => ("清除已添加", "Clear Added"),
        "git.current" => ("当前", "current"),
        "git.added" => ("已添加", "added"),
        "git.detachedHead" => ("分离的 HEAD", "Detached HEAD"),
        "git.noBranch" => ("无分支", "No branch"),
