...
每次授权召唤会先添加 👀 reaction，并创建一条“正在审查”的占位评论。Actions
日志包含上下文数量与大小、GitHub API 请求耗时，以及 Codex 的 JSONL 事件轨迹；
最终消息和诊断摘要会作为 artifact 保留 14 天。模型执行单独限制为 25 分钟，
即使失败或超时，工作流仍会更新占位评论并附上本次 Actions 运行链接。
