...
use crate::lsp::{
    self, ClientApplyServerMessageRequest, ClientChangeDocumentRequest, ClientCloseDocumentRequest,
...
impl LspHost {
    ...
    fn execute(&self, request: LspSessionCommandRequest) -> Result<LspSessionResponse, CoreError> {
        ...
        let response = match request.action {
            LspSessionAction::OpenDocument => {
                lsp::client_open_document(ClientOpenDocumentRequest {
                    state,
            ...
            LspSessionAction::ChangeDocument => {
                lsp::client_change_document(ClientChangeDocumentRequest {
                    state,
                    uri: required(request.uri, "uri")?,
                    text: required(request.text, "text")?,
                })
            }
            LspSessionAction::CloseDocument => {
                lsp::client_close_document(ClientCloseDocumentRequest {
                    state,
                    uri: required(request.uri, "uri")?,
                })
            }
            LspSessionAction::Shutdown => lsp::client_shutdown(ClientShutdownRequest { state }),
            LspSessionAction::Request => lsp::client_feature_request(ClientFeatureRequest {
                state,
                uri: required(request.uri, "uri")?,
            ...
            LspSessionAction::ApplyServerMessage => {
                lsp::client_apply_server_message(ClientApplyServerMessageRequest {
                    state,
    ...
    fn create(&self, request: LspSessionCommandRequest) -> Result<LspSessionResponse, CoreError> {
        let response = lsp::client_initialize(ClientInitializeRequest {
